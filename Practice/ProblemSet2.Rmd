...
# Problem Set 2: What determines wages?

What factors determine a person's wage is of just importance for policy. Today we are do some simple analyses using a dataset on individual characteristics and wage outcomes in 1980. You can find the CVS dataset under data in Canvas (Wages).
...
Dothenormaldescriptivestatisticsforthevariableswearegoingtouse.Anything stand out among these numbers?
...
The various variables are highly skewed and many missing data (NAs)

## 2. Correlation

Find the correlations between the variables we will use. What stands out here?
...
Run a regression with hours and IQ scoreas the explanatory variables. What are the coefficients for both and their interpretation (remember to include whether they are statistically significant).
...
This means that if `hours` differed by one unit, wage will differ by `r summary(fit1)$coefficients["hours", "Estimate"]` units, on average, while holding `IQ` constant.

This means that if `IQ` differed by one unit, wage will differ by `r summary(fit1)$coefficients["IQ", "Estimate"]` units, on average, while holding `hours` constant.  This is statistically significant at the 0.001 percent level.
...
Run the same regression as in Q3, but now add education as an additional explanatory variable. What are the coefficients and their interpretations (remember to include whether they are statistically significant).
...
Explain why the coefficients change between the two regression. Are the changes in the direction you would expect?
...
Run a regression with hours, IQs core, education, age, experience, and tenureas the explanatory variables. Interpret the results.
...
Explain why the coefficients change from the model in Q4.Does this make sense and why? (Hint: think what is behind the correlations you observed)
...
What does the F-test tell you about the model in Q6? What about the R2 and adjusted R2?
...
Can we drop tenure and hours? (Do a joint test). What are the results and how do we interpret them?
...
```{r}
Hnull <- c("tenure = exper")
linearHypothesis(fit3, Hnull)
```


Can you rule out that the effect of experience overall and tenure at the current job have the same effect on wage?(
...
If you have time, try to add parental education to the last model. Think about what changes. Also, try some joint hypotheses (for example, parental education have similar effect; difference between age and experience).
